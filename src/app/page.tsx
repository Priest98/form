...
export default function SurveyForm() {
  ...
  const ChoiceButton = ({ selected, label, onClick }: { selected: boolean, label: string, onClick: () => void }) => (
    <button
      onClick={onClick}
      className={`w-full text-left p-4 md:p-6 rounded-2xl border-2 transition-all duration-200 flex items-center justify-between group backdrop-blur-md ${
        selected 
  ...
  return (
    <div 
      className="min-h-screen bg-zinc-950 text-zinc-50 font-sans flex flex-col overflow-hidden selection:bg-indigo-500/30 bg-cover bg-center bg-no-repeat bg-fixed"
    ...
    >
      ...
      {currentStep > 0 && currentStep <= TOTAL_STEPS && (
        <header className="fixed top-0 left-0 right-0 p-6 md:p-8 z-50 flex items-center justify-between">
          ...
          <div className="flex flex-col items-end">
             <span className="text-sm font-medium text-zinc-400 mb-2">Question {currentStep} of {TOTAL_STEPS} • ~{timeRemaining} min</span>
             <div className="w-32 md:w-48 h-1.5 bg-zinc-800/80 rounded-full overflow-hidden">
                <motion.div 
                  initial={{ width: 0 }}
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 w-full max-w-3xl mx-auto relative z-10 min-h-[500px]">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* 0. INTRO */}
            {currentStep === 0 && (
              <div className="text-left space-y-8 bg-zinc-900/40 p-8 md:p-12 rounded-3xl border border-white/5 backdrop-blur-lg">
                <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-br from-white to-zinc-400 pb-2">
                ...
                <div className="pt-8">
                  ...
                  <p className="text-zinc-400 mt-4 text-sm font-medium">Press Enter ↵</p>
                </div>
              </div>
            )}
                  ...
                  {formData.role === "Other" && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <input 
                        ...
                        className="w-full mt-4 p-4 md:p-6 text-xl bg-black/20 backdrop-blur-md border-b-2 border-zinc-700 focus:border-indigo-500 text-white outline-none transition-colors rounded-t-xl"
                      />
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="space-y-6">
                ...
                <div className="mt-8">
                  <textarea 
                    ...
                    className="w-full p-4 md:p-6 text-xl md:text-2xl bg-black/20 backdrop-blur-md border-b-2 border-zinc-700 focus:border-indigo-500 text-white outline-none transition-colors resize-none placeholder-zinc-500 rounded-t-xl shadow-inner"
                  />
                  <p className="text-zinc-400 mt-4 text-sm font-medium">Tip: Press Cmd/Ctrl + Enter to submit</p>
                </div>
              </div>
            )}
            ...
            {currentStep === 9 && (
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-center mt-12 gap-2">
                  {[1, 2, 3, 4, 5].map(opt => (
                    <button
                      ...
                      className={`flex-1 aspect-square rounded-2xl flex flex-col items-center justify-center text-2xl font-bold transition-all backdrop-blur-md ${
                        formData.valueScore === opt.toString()
                          ? "bg-indigo-500 text-white scale-110 shadow-[0_0_30px_rgba(99,102,241,0.5)] border border-indigo-400"
                ...
                <div className="flex justify-between text-zinc-400 font-medium text-sm mt-4 px-2">
            ...
            {currentStep === 13 && (
              <div className="space-y-6">
                <h2 className="text-3xl md:text-4xl font-bold leading-tight">13. Would you like early access when we launch?</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                  {["Yes", "No"].map(opt => (
            ...
            {currentStep === 14 && (
              <div className="space-y-6">
                <h2 className="text-3xl md:text-4xl font-bold leading-tight">14. If you knew your social media would run automatically for the next 7 days, how likely would you be to spend one day each week creating all your content in advance?</h2>
                <div className="space-y-3 mt-8">
            ...
            {currentStep === 15 && (
              <div className="space-y-6">
                ...
                <div className="space-y-8 mt-8 bg-zinc-900/40 p-8 rounded-3xl border border-white/5 backdrop-blur-md">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full text-2xl bg-black/20 border-b-2 border-zinc-700 focus:border-indigo-500 text-white outline-none transition-colors py-3 px-4 placeholder-zinc-600 rounded-t-xl"
                    />
                  </div>
                  <div>
                    <label className="block text-zinc-300 font-medium mb-2">Email Address <span className="text-indigo-400">*</span></label>
                    <input 
                      type="email" 
                      autoFocus
                      required
                  ...
                  <div>
                    <label className="block text-zinc-300 font-medium mb-2">Phone / WhatsApp</label>
                    <input 
                      type="tel" 
            ...
            {currentStep === TOTAL_STEPS + 1 && (
              <div className="text-center space-y-8 animate-in zoom-in duration-700 bg-zinc-900/40 p-12 rounded-3xl border border-white/5 backdrop-blur-lg">
                <div className="mx-auto w-24 h-24 bg-green-500/20 text-green-400 rounded-full flex items-center justify-center mb-8 border border-green-500/30 shadow-[0_0_40px_rgba(34,197,94,0.3)]">
                ...
                <div className="pt-8">
                  <button 
                    ...
                    className="inline-flex h-14 items-center justify-center rounded-full bg-zinc-800 px-10 font-semibold text-white hover:bg-zinc-700 transition-colors border border-white/10"
                  >
                    Take Survey Again
                  </button>
                </div>
      ...
      {currentStep > 0 && currentStep <= TOTAL_STEPS && (
        <footer className="fixed bottom-0 left-0 right-0 p-6 md:p-8 z-50 flex items-center justify-between bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-transparent">
          <div className="flex items-center space-x-4">
             <button 
               onClick={handleBack}
               className="w-12 h-12 rounded-full bg-zinc-900/80 flex items-center justify-center text-zinc-300 hover:bg-zinc-800 hover:text-white transition-colors border border-white/10 backdrop-blur-md"
             >
          ...
          <div className="flex items-center space-x-4">
             ...
             <button 
               ...
               className={`h-14 px-8 rounded-full font-bold text-lg flex items-center transition-all shadow-lg ${
                 isStepValid() 
                   ? "bg-indigo-600 text-white hover:bg-indigo-500 hover:scale-105 shadow-indigo-600/20" 
                   : "bg-zinc-800/80 text-zinc-500 cursor-not-allowed border border-white/5 backdrop-blur-md"
               }`}
             >
